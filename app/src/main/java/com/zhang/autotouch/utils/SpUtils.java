...
public class SpUtils {
   ...
   * 只保存一个主触控点，简化使用流程
   */
  public static void setMainTouchPoint(Context context, TouchPoint touchPoint) {
    if (touchPoint == null) {
      setTouchPoints(context, new ArrayList<TouchPoint>());
      return;
    }
    setTouchPoints(context, Collections.singletonList(touchPoint));
  }

  public static TouchPoint getMainTouchPoint(Context context) {
    List<TouchPoint> touchPoints = getTouchPoints(context);
